...
const CarouselCards = ({ item }) => {
    return (
        <View className="bg-blue-700 rounded-3xl p-6 mx-8 space-x-2 space-y-2">
            <View className="flex flex-row justify-between ">
                <Image source={require("../../images/avatar.jpg")} className="rounded-md w-24 h-28 mr-12" />
                <View className="">
                    <Text className="font-medium text-white mb-1">{item.doctor}</Text>
                    <Text className="font-medium text-gray-200 text-sm">{item.specialist}</Text>
                    <Text className="font-medium text-gray-200 text-sm mb-2">{item.place}</Text>
                    <View className="p-2 bg-white rounded-lg flex flex-row space-x-2 items-center">
                        <AntDesign name="star" size={18} color="#eab308" />
                        <Text className="text-md text-blue-700 font-medium">{item.star}</Text>
                    </View>
                </View>
            </View>
